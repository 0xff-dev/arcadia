...
var (
	...
	VersionedDatasetFinalizer         = Group + "/finalizers"
)
...
func CopyedFileGroup2Status(instance *VersionedDataset) (bool, []DatasourceFileStatus) {
	...
	for _, fg := range instance.Spec.FileGroups {
		...
		for _, fp := range fg.Paths {
			fileGroup[key] = append(fileGroup[key], fp)
		}
	}
...
func UpdateFileStatus(ctx context.Context, instance *VersionedDataset, datasource, srcPath string, syncStatus FileProcessPhase) error {
