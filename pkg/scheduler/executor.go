...
type executor struct {
	...
	deleteFileGroups []v1alpha1.FileStatus
}
...
func newExecutor(ctx context.Context, c client.Client, minioClient *minio.Client, instance *v1alpha1.VersionedDataset) butcher.Executor[JobPayload] {
	_, deleteFileGroups := v1alpha1.CopyedFileGroup2Status(minioClient, instance)
	klog.V(4).Infof("[Debug] status is: %+v\ndelete filegroups: %+v\n", instance.Status.Files, deleteFileGroups)
	klog.V(4).Infof("[Debug] client is nil: %v\n", c == nil)
	return &executor{instance: instance, deleteFileGroups: deleteFileGroups, client: c, minioClient: minioClient}
}
...
func (e *executor) generateJob(ctx context.Context, jobCh chan<- JobPayload, datasourceFiles []v1alpha1.FileStatus, removeAction bool) error {
	for _, fs := range datasourceFiles {
		...
		} else if fs.Kind == "VersionedDataset" {
...
func (e *executor) GenerateJob(ctx context.Context, jobCh chan<- JobPayload) error {
	if err := e.generateJob(ctx, jobCh, e.instance.Status.Files, false); err != nil {
...
func (e *executor) OnFinish(ctx context.Context, job JobPayload, err error) {
	...
	klog.V(4).Infof("[Debug] change the status of file %s/%s to %s", job.SourceName, job.Src, syncStatus)
	if err = v1alpha1.UpdateFileStatus(ctx, e.instance, job.SourceName, job.Src, syncStatus, errMsg); err != nil {
