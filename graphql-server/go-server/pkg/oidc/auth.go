...
// client-go does cache the user's authentication information,
// for the case of needing different users to access, it will lead to the token is the same,
// and then can't control the authority, at the same time, we build the client does not provide refresh token,
// resulting in the token expired when can't be refreshed, so here to remove the cache.
